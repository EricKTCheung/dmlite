...
static const char* STMT_GET_FILE_BY_ID =
    "SELECT fileid, parent_fileid, guid, name, filemode, nlink, owner_uid, gid,\
        ...
        WHERE fileid = ?";
static const char* STMT_GET_FILE_BY_GUID =
    "SELECT fileid, parent_fileid, guid, name, filemode, nlink, owner_uid, gid,\
            filesize, atime, mtime, ctime, fileclass, status,\
            csumtype, csumvalue, acl\
        FROM Cns_file_metadata\
...
static const char* STMT_GET_LIST_FILES =
    "SELECT fileid, parent_fileid, guid, name, filemode, nlink, owner_uid, gid,\
          filesize, atime, mtime, ctime, fileclass, status,\
          csumtype, csumvalue, acl\
        FROM Cns_file_metadata \
        WHERE parent_fileid = ?";
static const char* STMT_GET_SYMLINK =
    "SELECT fileid, linkname FROM Cns_symlinks WHERE fileid = ?";
static const char* STMT_GET_USERINFO_BY_NAME =
...
static const char* STMT_GET_USERINFO_BY_UID =
    "SELECT userid, username, user_ca, banned\
        FROM Cns_userinfo\
        WHERE userid = ?";
static const char* STMT_GET_GROUPINFO_BY_NAME =
    "SELECT gid, groupname, banned\
        FROM Cns_groupinfo\
        WHERE groupname = ?";
static const char* STMT_GET_GROUPINFO_BY_GID =
    "SELECT gid, groupname, banned\
        FROM Cns_groupinfo\
        WHERE gid = ?";
static const char* STMT_GET_FILE_REPLICAS =
    "SELECT rowid, fileid, nbaccesses,\
            atime, ptime, ltime,\
            status, f_type, poolname, host, fs, sfn\
        FROM Cns_file_replica\
        WHERE fileid = ?";
static const char* STMT_GET_REPLICA_BY_URL =
    "SELECT rowid, fileid, nbaccesses,\
            atime, ptime, ltime,\
            status, f_type, poolname, host, fs, sfn\
        FROM Cns_file_replica\
        WHERE sfn = ?";
static const char* STMT_GET_COMMENT =
    "SELECT comments\
        FROM Cns_user_metadata\
        WHERE u_fileid = ?";
static const char* STMT_SET_GUID =
    "UPDATE Cns_file_metadata\
        SET guid = ?, ctime = UNIX_TIMESTAMP()\
        WHERE fileid = ?";
static const char* STMT_SET_COMMENT =
    "UPDATE Cns_user_metadata\
        SET comments = ?\
        WHERE u_fileid = ?";
static const char* STMT_INSERT_COMMENT =
    "INSERT INTO Cns_user_metadata\
          (u_fileid, comments)\
        VALUES\
          (?, ?)";
static const char* STMT_INSERT_FILE =
    "INSERT INTO Cns_file_metadata\
        ...
        VALUES\
          (?, ?, ?, ?, ?, ?, ?,\
           ...
           ?, ?, ?)";
static const char* STMT_INSERT_SYMLINK =
    "INSERT INTO Cns_symlinks\
          (fileid, linkname)\
        VALUES\
          (?, ?)";
static const char* STMT_SELECT_UNIQ_ID_FOR_UPDATE =
    "SELECT id FROM Cns_unique_id FOR UPDATE";
static const char* STMT_UPDATE_UNIQ_ID =
    "UPDATE Cns_unique_id SET id = ?";
static const char* STMT_INSERT_UNIQ_ID =
    "INSERT INTO Cns_unique_id (id) VALUES (?)";
static const char* STMT_DELETE_FILE =
    "DELETE FROM Cns_file_metadata WHERE fileid = ?";
static const char* STMT_DELETE_COMMENT =
    "DELETE FROM Cns_user_metadata WHERE u_fileid = ?";
static const char* STMT_DELETE_SYMLINK =
    "DELETE FROM Cns_symlinks WHERE fileid = ?";
static const char* STMT_NLINK_FOR_UPDATE =
    "SELECT nlink FROM Cns_file_metadata WHERE fileid = ? FOR UPDATE";
static const char* STMT_UPDATE_NLINK =
    "UPDATE Cns_file_metadata\
        SET nlink = ?, mtime = UNIX_TIMESTAMP(), ctime = UNIX_TIMESTAMP()\
        WHERE fileid = ?";
static const char* STMT_UPDATE_PERMS =
    "UPDATE Cns_file_metadata\
    ...
    "INSERT INTO Cns_file_replica\
        ...
        VALUES\
          (?, 0,\
           ...
           ?, ?, ?,\
           ?, ?, ?, ?, ?)";
static const char* STMT_TRUNCATE_FILE =
    "UPDATE Cns_file_metadata\
        SET filesize = 0, ctime = UNIX_TIMESTAMP()\
        WHERE fileid = ?";
static const char* STMT_GET_UNIQ_UID_FOR_UPDATE =
    "SELECT id FROM Cns_unique_uid FOR UPDATE";
static const char* STMT_GET_UNIQ_GID_FOR_UPDATE =
    "SELECT id FROM Cns_unique_gid FOR UPDATE";
static const char* STMT_UPDATE_UNIQ_UID =
    "UPDATE Cns_unique_uid SET id = ?";
static const char* STMT_UPDATE_UNIQ_GID =
    "UPDATE Cns_unique_gid SET id = ?";
static const char* STMT_INSERT_UNIQ_UID =
    "INSERT INTO Cns_unique_uid (id) VALUES (?)";
static const char* STMT_INSERT_UNIQ_GID =
    "INSERT INTO Cns_unique_gid (id) VALUES (?)";
static const char* STMT_INSERT_USER =
    "INSERT INTO Cns_userinfo\
          (userid, username, user_ca, banned)\
        VALUES\
          (?, ?, ?, ?)";
static const char* STMT_INSERT_GROUP =
    "INSERT INTO Cns_groupinfo\
          (gid, groupname, banned)\
        VALUES\
          (?, ?, ?)";
static const char* STMT_CHANGE_NAME =
    "UPDATE Cns_file_metadata\
        SET name = ?, ctime = UNIX_TIMESTAMP()\
        WHERE fileid = ?";
static const char* STMT_CHANGE_PARENT =
    "UPDATE Cns_file_metadata\
        SET parent_fileid = ?, ctime = UNIX_TIMESTAMP()\
        WHERE fileid = ?";
static const char* STMT_UTIME =
    "UPDATE Cns_file_metadata\
        SET atime = ?, mtime = ?, ctime = UNIX_TIMESTAMP()\
        WHERE fileid = ?";
static const char* STMT_UPDATE_REPLICA =
