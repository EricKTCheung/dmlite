...
Statement::Statement(MYSQL* conn, const std::string& db, const char* query) throw (DmException):
  nFields_(0), result_(NULL), result_null_(NULL), status_(STMT_CREATED)
{
...
Statement::~Statement() throw ()
{
  ...
  // Free array of null result indicators
  if (this->result_null_ != NULL) {
    delete [] this->result_null_;
  }
...
unsigned long Statement::execute(void) throw (DmException)
{
  ...
  if (meta == NULL) {
    ...
    this->result_null_ = new my_bool[this->nFields_];
    std::memset(this->result_null_, 0, sizeof(my_bool) * this->nFields_);
...
void Statement::zeroNullResults()
{
  for (unsigned long l=0;l<this->nFields_;++l) {
    MYSQL_BIND *b = &this->result_[l];
    if (!b->is_null || !*b->is_null) continue;
    switch(b->buffer_type) {
      case MYSQL_TYPE_SHORT:
        ASSIGN_POINTER_TYPECAST(short, b->buffer, 0);
        break;
      case MYSQL_TYPE_LONG:
        ASSIGN_POINTER_TYPECAST(int, b->buffer, 0);
        break;
      case MYSQL_TYPE_LONGLONG:
        ASSIGN_POINTER_TYPECAST(long long, b->buffer, 0);
        break;
      case MYSQL_TYPE_STRING:
        if (b->buffer_length) {
          ASSIGN_POINTER_TYPECAST(char, b->buffer, '\0');
        }
        break;
